...
final class ImageRegistry {
    ...
    static ImageIcon getIcon(String name) {
        return CACHE.getOrDefault(name, CACHE.get(DEFAULT_KEY));
    }

    private static ImageIcon load(String path) {
        if (path == null || path.isBlank()) return CACHE.get(DEFAULT_KEY);

        ImageIcon icon = new ImageIcon(path);

        if (icon.getIconWidth() <= 0) return CACHE.get(DEFAULT_KEY);
